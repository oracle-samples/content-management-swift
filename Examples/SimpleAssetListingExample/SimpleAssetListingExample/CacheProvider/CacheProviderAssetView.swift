...
/// Downloads images using the `downloadAsync` invocation verb, which returns a URL in its result property
struct CacheProviderAssetView: View {
    @ObservedObject var model: CacheProviderAssetModel
    
    init(_ asset: Asset) {
        self.model = CacheProviderAssetModel(asset)
    }
...
@MainActor
class CacheProviderAssetModel: ObservableObject {
        ...
        do {
            let downloadResult = try await DeliveryAPI
                .downloadThumbnail(identifier: self.asset.identifier,
                                   ...
                                   cacheProvider: GalleryCacheProvider(),
                                   cacheKey: self.asset.identifier)
                .downloadAsync(progress: nil)
            
            let image = UIImage(contentsOfFile: downloadResult.result.path)
