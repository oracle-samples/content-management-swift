...
/// Downloads images using the `downloadImageAsync` invocation verb, which returns a UIImage (on iOS) in its result property
struct AsyncAwaitAssetView: View {
    @ObservedObject var model: AsyncAwaitAssetModel
    
    init(_ asset: Asset) {
        self.model = AsyncAwaitAssetModel(asset)
    }
...
@MainActor
class AsyncAwaitAssetModel: ObservableObject {
    ...
    func fetchThumbnail() async {
        ...
        do {
            ...
            self.thumbnail = UIImage(contentsOfFile: downloadResult.result.path)  
