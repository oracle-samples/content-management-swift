...
/**
 Protocol used by `ImplementsDownload` so that callers can provide a URL-based cache interface when downloading assets of any type.
...
public protocol CacheProvider {
    ...
    /// Provide your own method to return a URL if an entry is found in cache matching the specified key.
    ...
    /// - returns: URL? representing the file URL of the item matching the key.  If you return a non-nil URL, then that return value will be used and the server call will be bypassed
    ...
    ///
    /// - parameter key: String value representing the key in the cache
    /// - returns: URL representing the file URL of the item matching the key. If none exists, throw an error so that the entire pipeline can fail. Otherwise return a URL so that the pipeline can return success
    ...
    /// - parameter file: The URL of the file that should be stored in cache
    ...
    /// - returns: The new URL representing the persisted file location
    /// - throws: Error if issues exist reading or copying the file at the URL
